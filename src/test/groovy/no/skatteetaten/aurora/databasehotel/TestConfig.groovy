...
class TestConfig {
  static Environment loadTestConfig() {
    def environment = new MockEnvironment().with {
      List<PropertySource<?>> propertySource = new YamlPropertySourceLoader().
          load("test-properties", new ClassPathResource("application.yml"))
      getPropertySources().addFirst(propertySource.first())
